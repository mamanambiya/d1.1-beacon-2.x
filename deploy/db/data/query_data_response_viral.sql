...
CREATE OR REPLACE FUNCTION public.query_data_response_viral(
	_variant_type text,
	_start integer,
	_start_min integer,
	_start_max integer,
	...
	_dataset_ids text,
	_filters text)
    RETURNS TABLE(id text, variant_composite_id text, chromosome text, variant_id text, reference text, alternate text, start integer, "end" integer, variant_type text, variant_cnt integer, call_cnt integer, sample_cnt integer, matching_sample_cnt integer, frequency numeric, dataset_id integer, sample_id text) 
    LANGUAGE 'plpgsql'
...
BEGIN
	...
	_query = '
		SELECT
			CONCAT(bdat.dataset_id, bdat.chromosome, bdat.variant_id, bdat.reference, bdat.alternate, bdat.start, bdat.end, bdat.type) AS id,
			CONCAT(bdat.chromosome, bdat.variant_id, bdat.reference, bdat.alternate, bdat.start, bdat.end, bdat.type) AS variant_composite_id,
			...
			bdat.end, 
			bdat.type::TEXT, 
			--max(bdat.sv_length) as sv_length, 
		...
		LEFT JOIN (SELECT data_id, stable_id as sample_id FROM public.beacon_data_sample_table ds
					JOIN beacon_sample_table s ON ds.sample_id = s.id) as bsam ON bsam.data_id = bdat.id
		WHERE';
	...
	GROUP BY CONCAT(bdat.dataset_id, bdat.chromosome, bdat.variant_id, bdat.reference, bdat.alternate, bdat.start, bdat.end, bdat.type),
		bdat.chromosome, bdat.variant_id, bdat.reference, bdat.alternate, 
		bdat.start, bdat.end, bdat.type, bdat.dataset_id, sample_id
	ORDER BY bdat.chromosome, bdat.variant_id, bdat.reference, bdat.alternate, 
		bdat.start, bdat.end, bdat.type, bdat.dataset_id';
